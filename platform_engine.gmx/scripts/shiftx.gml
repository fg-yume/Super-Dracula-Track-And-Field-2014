...
if( argument0 != 0 ){
    ...
    else if( !place_meeting(x+argument0,y-abs(argument0*2)-2,oWall) ){
        x += argument0;
        y -= abs(argument0*2)+2;
        move_up_slope = 1;
    }
    ...
    if( !fallthrough && place_meeting(x,y,oFloor) && !place_meeting(x+argument0,y-abs(argument0*2)-2,oFloor) ){
        y -= abs(argument0*2)+2;
        move_up_slope = 1;
    }
    
    //move character down if just above the ground (so he can walk down slopes
    if( (place_meeting(x,y+abs(argument0*2)+2,oFloor) && onground) || move_up_slope ){
