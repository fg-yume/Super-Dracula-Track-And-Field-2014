...
if(argument0 < 0)
{
    ...
    if( !place_meeting(x,y+argument0,oWall) ){
        y += argument0;
    }
    //move adjacent to wall
    else{
        while( !place_meeting(x,y+sign(argument0),oWall) ){
            y += sign(argument0);
        }
        ymomentum = 0;
    }
...
{
    //if new location is free, move there
    if( !place_meeting(x,y+argument0,oFloor) || ( place_meeting(x,y,oFloor) && !place_meeting(x,y+argument0,oWall) ) ){
        y += argument0;
    }
    else{
        ...
        //move down to the nearest platform
            ...
            while( !place_meeting(x,y+sign(argument0),oFloor) ){
                y += sign(argument0);
            }    
        }
